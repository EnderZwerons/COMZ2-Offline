...
public class CoopRoomUserItem : MonoBehaviour
{
	...
	public bool ResetUserItem(TNetUser user, bool show_kick_button)
	{
		...
		if (cur_user.ContainsVariable(TNetUserVarType.RoomState))
		{
			...
			user_name.Text = sFSObject.GetUtfString("nickname");
			int @int = sFSObject.GetInt("avatarLv");
			user_icon.texture = GameConfig.Instance.AvatarConfig_Set[(AvatarType)sFSObject.GetInt("avatarType")].avatar_name + "_0" + @int + "_icon";
			user_combat_val.Text = "Day:" + sFSObject.GetInt("day");
			if (cur_user.IsItMe)
			{
